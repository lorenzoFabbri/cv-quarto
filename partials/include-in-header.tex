% Include-in-header for CV (XeLaTeX-friendly and biblatex-ready)
% Adapted from:
...
% --------------------------------------------------------------------
\usepackage{fontspec} % XeLaTeX engine in Quarto
% set a main font here if desired, e.g.:
...
% --------------------------------------------------------------------
% Colours & hyperlinks (xcolor before hyperref)
% --------------------------------------------------------------------
...
% Basic layout
% --------------------------------------------------------------------
% Explicit paragraph settings (no parskip package to avoid conflicts)
...
% --------------------------------------------------------------------
\usepackage{datetime} % datetime2 may be used if you need more variants
\usepackage{fancyhdr}
\usepackage{lastpage}
\pagestyle{fancy}
...
\usepackage[all,defaultlines=2]{nowidow} % widow/orphan control
...
% --------------------------------------------------------------------
% Bibliography: ensure biblatex + biber interplay cleanly
% - Guarded loading: if biblatex is already loaded by Quarto, do not reload
% - Ensure the bibliography file is added
% --------------------------------------------------------------------
\usepackage{etoolbox} % used by name-highlighting macros (and for conditional logic)

\makeatletter
% If biblatex is not already loaded, load it with options consistent with YAML
...
% Slightly reduce bibliography spacing (works with biblatex)
...
% (these rely on etoolbox; keep them after etoolbox is loaded)
% --------------------------------------------------------------------
\def\makenamesetup{
  \def\bibnamedelima{~}
  \def\bibnamedelimb{ }
